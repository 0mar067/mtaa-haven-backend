...
def seed_database():
    ...
    users = []
    for _ in range(20):
        user = User(email=fake.email(), password_hash=fake.password(), first_name=fake.first_name(), last_name=fake.last_name(), phone=fake.phone_number(), user_type=random.choice(list(UserType)))
        users.append(user)
        db.session.add(user)
    db.session.commit()
    ...
    properties = []
    for _ in range(15):
        prop = Property(title=fake.catch_phrase(), description=fake.text(), address=fake.address(), city=fake.city(), rent_amount=Decimal(str(random.randint(500, 3000))), bedrooms=random.randint(1, 4), bathrooms=random.randint(1, 3), landlord_id=random.choice(landlords).id if landlords else users[0].id)
    ...
    # Create payments
    for _ in range(30):
        payment = Payment(amount=Decimal(str(random.randint(500, 3000))), payment_date=fake.date_time_this_year(), due_date=fake.date_time_this_year(), user_id=random.choice(users).id, property_id=random.choice(properties).id)
        db.session.add(payment)
    
    # Create issues
    for _ in range(25):
        issue = Issue(title=fake.sentence(nb_words=4), description=fake.text(), issue_type=random.choice(list(IssueType)), reporter_id=random.choice(users).id, property_id=random.choice(properties).id)
        db.session.add(issue)
